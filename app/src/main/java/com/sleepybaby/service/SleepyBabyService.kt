package com.sleepybaby.service
...
import com.sleepybaby.engine.CryDetectionEngine
...
class SleepyBabyService : Service() {
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            when (val backend = cryDetectionEngine.loadOnDeviceClassifier()) {
                OnDeviceCryClassifier.Backend.ONNX ->
                    Log.d("SleepyBabyService", "Loaded ONNX cry classifier")
                OnDeviceCryClassifier.Backend.TFLITE ->
                    Log.d("SleepyBabyService", "Loaded TFLite cry classifier")
                OnDeviceCryClassifier.Backend.ENERGY ->
                    Log.i(
                        "SleepyBabyService",
                        "Using local energy-based classifier"
                    )
                OnDeviceCryClassifier.Backend.UNINITIALIZED ->
                    Log.e("SleepyBabyService", "Failed to initialize any cry classifier")
            }
        }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Cry detection automation service"
                setShowBadge(false)
                enableLights(false)
    ...
    private fun createNotification(state: AutomationState): Notification {
        ...
        val contentText = when (state) {
            is AutomationState.Listening -> "Listening for baby cries..."
            is AutomationState.CryingPending -> "Cry detected (${state.consecutiveCrySeconds}s)"
            is AutomationState.Playing -> "Playing soothing sounds"
            is AutomationState.FadingOut -> "Fading out..."
            is AutomationState.Stopped -> "Service stopped"
        }

        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("SleepyBaby Active")
